...
if st.session_state.gdf is not None:
    ...
    with tab6:
        st.markdown("### Spatial Operations")
        st.markdown("<br>", unsafe_allow_html=True)
        
        operation = st.selectbox(
            "Select Spatial Operation",
            ["Buffer", "Centroid", "Dissolve", "Spatial Join (Multi-Dataset)"]
        )
        ...
        if operation == "Buffer":
            ...
            st.info("Create a buffer zone around features")
            ...
            if st.button("Create Buffer"):
                with st.spinner("Creating buffer..."):
                    try:
                        ...
                        buffered['geometry'] = gdf_metric.geometry.buffer(buffer_distance, resolution=buffer_resolution)
                        
                        # Convert back to original CRS
                        buffered = buffered.to_crs(gdf.crs)
                        
                        # Store result
                        st.session_state.spatial_operation_result = buffered
                        
                        # Add to datasets
                        result_name = f"{st.session_state.active_dataset}_buffer_{buffer_distance}m"
                        st.session_state.datasets[result_name] = buffered
                        
                        st.success(f"✅ Buffer created! Added as '{result_name}'")
                        
                        # Show preview
                        st.markdown("##### Result Preview")
                        st.dataframe(buffered.head(), use_container_width=True)
                        
                    except Exception as e:
                        st.error(f"Error creating buffer: {str(e)}")
        
        elif operation == "Centroid":
            st.markdown("#### Centroid Calculation")
            st.info("Calculate centroids for all features")
            
            if st.button("Calculate Centroids"):
                with st.spinner("Calculating centroids..."):
                    try:
                        centroids = gdf.copy()
                        centroids['geometry'] = gdf.geometry.centroid
                        
                        st.session_state.spatial_operation_result = centroids
                        
                        result_name = f"{st.session_state.active_dataset}_centroids"
                        st.session_state.datasets[result_name] = centroids
                        
                        st.success(f"✅ Centroids calculated! Added as '{result_name}'")
                        
                        st.markdown("##### Result Preview")
                        st.dataframe(centroids.head(), use_container_width=True)
                        
                    except Exception as e:
                        st.error(f"Error calculating centroids: {str(e)}")
        
        
        elif operation == "Dissolve":
            st.markdown("#### Dissolve Features")
            st.info("Merge features based on an attribute")
            
            # Get non-geometry columns
            dissolve_cols = [col for col in gdf.columns if col != 'geometry']
            
            if dissolve_cols:
                dissolve_by = st.selectbox("Dissolve by attribute", dissolve_cols)
                
                if st.button("Dissolve"):
                    with st.spinner("Dissolving features..."):
                        try:
                            dissolved = gdf.dissolve(by=dissolve_by, aggfunc='first')
                            dissolved = dissolved.reset_index()
                            
                            st.session_state.spatial_operation_result = dissolved
                            
                            result_name = f"{st.session_state.active_dataset}_dissolved"
                            st.session_state.datasets[result_name] = dissolved
                            
                            st.success(f"✅ Features dissolved! Reduced from {len(gdf)} to {len(dissolved)} features")
                            st.info(f"Added as '{result_name}'")
                            
                            st.markdown("##### Result Preview")
                            st.dataframe(dissolved.head(), use_container_width=True)
                            
                        except Exception as e:
                            st.error(f"Error dissolving features: {str(e)}")
            else:
                st.warning("No attributes available for dissolving")
        
        elif operation == "Spatial Join (Multi-Dataset)":
            st.markdown("#### Spatial Join")
            st.info("Join two datasets based on spatial relationship")
            
            if len(st.session_state.datasets) >= 2:
                available_datasets = [name for name in st.session_state.datasets.keys() if name != st.session_state.active_dataset]
                
                col1, col2 = st.columns(2)
                
                with col1:
                    st.markdown(f"**Left Dataset:** {st.session_state.active_dataset}")
                
                with col2:
                    right_dataset = st.selectbox("Right Dataset", available_datasets)
                ...
                how_join = st.selectbox(
                    "Join Type",
                    ["inner", "left", "right"]
                )
                
                if st.button("Perform Spatial Join"):
                    with st.spinner("Performing spatial join..."):
                        try:
                            left_gdf = gdf
                            right_gdf = st.session_state.datasets[right_dataset]
                            
                            # Ensure same CRS
                            if left_gdf.crs != right_gdf.crs:
                                right_gdf = right_gdf.to_crs(left_gdf.crs)
                            
                            joined = gpd.sjoin(left_gdf, right_gdf, how=how_join, predicate=join_type)
                            
                            st.session_state.spatial_operation_result = joined
                            
                            result_name = f"{st.session_state.active_dataset}_join_{right_dataset}"
                            st.session_state.datasets[result_name] = joined
                            
                            st.success(f"✅ Spatial join completed! {len(joined)} features in result")
                            st.info(f"Added as '{result_name}'")
                            ...
                            st.error(f"Error performing spatial join: {str(e)}")
            else:
                st.warning("Please load at least 2 datasets to perform spatial join")
        
        # Export operation result
        if st.session_state.spatial_operation_result is not None:
            st.markdown("---")
            st.markdown("#### Export Operation Result")
            
            col1, col2 = st.columns(2)
            
            with col1:
                result_csv = st.session_state.spatial_operation_result.to_csv(index=False)
                st.download_button(
                    label="Download Result (CSV)",
                    data=result_csv,
                    file_name=f"spatial_operation_result_{datetime.now().strftime('%Y%m%d_%H%M%S')}.csv",
                    mime="text/csv"
                )
            
            with col2:
                result_geojson = st.session_state.spatial_operation_result.to_json()
                st.download_button(
                    label="Download Result (GeoJSON)",
                    data=result_geojson,
                    file_name=f"spatial_operation_result_{datetime.now().strftime('%Y%m%d_%H%M%S')}.geojson",
